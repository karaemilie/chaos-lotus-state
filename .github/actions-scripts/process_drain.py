...
MVP scope: ZONES completions only. Other source types stay queued for
Claude to handle manually until we verify the auto path works.
"""
...
def main():
    ...
    commit_msg = f"🤖 Auto-process: {len(processed_uids)} zone completion(s) stamped"
    result = save_beast(new_beast_bytes, beast_sha, commit_msg)
    print(f"   Committed: {result['commit']['sha'][:12]}")

    # 6. Rewrite drain.json — remove processed items, keep the rest
    ...
    new_drain = {
        **drain,
        ...
        "lastAutoProcessed": processed_uids,
    }
